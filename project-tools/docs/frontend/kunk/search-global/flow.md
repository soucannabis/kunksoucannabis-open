...
Se o hit for responsável com paciente(s) no funil, opcional mostrar paciente vinculado (legado usava `responsible_for`; OSS lista via `patients` / primeiro paciente se necessário — **sem** depender de ativo global).
...
Legado abria triagem pública `/triagem?t=`. No OSS preferir a **fila autenticada** `/app/acolhimento/triagem?t=` (operador); manter público só se o produto ainda expuser tela pública.
