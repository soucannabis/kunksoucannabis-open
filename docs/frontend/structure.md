...
1. **Um repositório de produto** — apps e pacotes no mesmo tree do OSS (o `cadastramento/` histórico com `.git` próprio deixa de ser o modelo).
...
4. **Schema novo no cliente** — nomes de campo do schema alvo (`associate_name`, etc.), nunca os typos o schema de origem (`emiiter_rg_associate`).
...
- Componentes sem acoplamento a Bootstrap ou MUI anteriores
...
- **Proibido** embutir tokens de o schema de origem, API keys ou chaves de criptografia no bundle (dívida anteriores)
...
| Em paralelo | Manter `cadastramento/` histórico em produção até cutover |
...
Stack visual do **registration**: preservar Bootstrap + CSS custom anteriores (ver [cadastramento/ui-ux.md](./cadastramento/ui-ux.md)), aprimorando layout sem trocar a identidade.
