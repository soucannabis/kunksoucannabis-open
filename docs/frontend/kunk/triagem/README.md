...
> Schema alvo: tabela `reception` ([Reception.md](../../schema-origem/collections/Reception.md) + `target-schema.sql`).

## Objetivo

Recriar a triagem operacional com:

1. **Mesma lógica de fila** anteriores (lista de `reception`, sidebar de status, redirecionamento para pedidos/serviços)
...
| [ui-ux.md](./ui-ux.md) | Página operacional em `apps/kunk` (layout histórico sem Utalk/Beeviral) |
...
| Contabilizar conclusão em versões anteriores (`completion_reason`) | Manter histórico de doações na UI da triagem |
