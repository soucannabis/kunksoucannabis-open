...
3. **Sidebar lateral de status** (configurados no admin):
 - Cada status mostra **contagem** dos registros com aquele status (em versões anteriores).
 - Status padrão de entrada: **Espera**.
 - Status terminal padrão: **Concluído** .
...
| Com `associate_code` | Navega para pedido/serviço com query `u={associate_code}` (padrãversões anteriores) |

### Contabilização (em versões anteriores)
...
2. Marcar como concluída:
 ...
 - `completion_reason` = `Pedido` \| `Serviço` 
3. A contagem da sidebar atualiza: sai de Espera (ou status intermediário) e entra em Concluído.

**Redirect para serviços com finalização imediata** (opcional, espelhananteriores `appointmentRedirect`):
...
- Quando **habilitado**: abrir painel/modal do associado (dados + documentos), no espírito do `UserModal` histórico, **sem** aba de histórico de doações.
