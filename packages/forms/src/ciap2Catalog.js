/** Full CIAP-2 catalog (ported from kunksoucannabis legacy). */
...
/** Flatten category → subcategory rows with category label. */
export function flattenCiapOptions(options = CIAP2_OPTIONS) {
  return options.flatMap((cat) =>
...
const BY_CODE = new Map(flattenCiapOptions().map((o) => [o.value, o]));

/** Normalize DB/API value to string[] of codes. */
export function normalizeCiapCodes(value) {
  if (Array.isArray(value)) {
    return value
      .map((v) => (typeof v === 'object' && v && v.value != null ? String(v.value) : String(v || '')))
      .map((s) => s.trim())
      .filter(Boolean);
  }
...
/** Resolve codes to { value, label, category }[]. */
export function resolveCiapCodes(value) {
  return normalizeCiapCodes(value).map((code) => BY_CODE.get(code) || { value: code, label: code, category: '' });
}

/** Persist as semicolon-separated codes. */
export function serializeCiapCodes(value) {
  const codes = normalizeCiapCodes(value);
